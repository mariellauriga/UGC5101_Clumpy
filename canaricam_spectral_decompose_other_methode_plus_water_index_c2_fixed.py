...
def func(p, ccam_x):
  c1, c3, c4, c5, c6, c7 = p
  return c1*ccam_x**(2.0)*np.exp(-c3*f_ext)*np.exp(-c6*t_hielo) + c4*f_sb*np.exp(-c5*f_ext)*np.exp(-c7*t_hielo)
...
y_cont = p[0]*ccam_x**(2.0)*np.exp(-p[1]*f_ext)*np.exp(-p[4]*t_hielo)
y_sb = p[2]*f_sb*np.exp(-p[3]*f_ext)*np.exp(-p[5]*t_hielo)
y_tot = y_cont + y_sb
