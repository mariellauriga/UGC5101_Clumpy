...
#file500 = open('h2op30k_new_spitzer.dat','w+')
#print >>file500, '#lambda(um)Rest-Frame', 'f_nu(mJy)'
#for i in range(0,len(spit_x)):
#  print >>file500, spit_x[i], t_hielo[i]
#file500.close() 
...
def func(p, spit_x): 
  c1, c3, c4, c5, c6, c7 = p
  return c1*spit_x**(c2)*np.exp(-c3*f2)*np.exp(-c6*t_hielo) + c4*f1*np.exp(-c5*f2)*np.exp(-c7*t_hielo)

def err(p, spit_y, spit_x):
  return (spit_y - func(p, spit_x))/sigma_obs_spec

#p0 =[10., 2., 18., 2., 38., 0.05, 15]
p0 =[10., 40., 0.5, 10., 11., 0.05]
bounds = [(0.5,None), (0,None), (0,None), (0,None), (0, None), (0, None)]

p, cov_x, infodic, mesg, ier = leastsqbound(err, p0, args=(spit_y, spit_x), bounds = bounds, full_output=True, maxfev=20000)
...
y_cont = a1*spit_x**(a2)*np.exp(-a3*f2)*np.exp(-a6*t_hielo)

y_sb = a4*f1*np.exp(-a5*f2)*np.exp(-a7*t_hielo)
...
print 'a3=',p[1], '+\-',sigma[1]
print 'a4=',p[2], '+\-',sigma[2]
print 'a5=',p[3], '+\-',sigma[3]
print 'a6=',p[4], '+\-',sigma[4]
print 'a7=',p[5], '+\-',sigma[5]
...
#Errors propagation
dfunc_c1 = spit_x**(a2)*np.exp(-a3*f2)*np.exp(-a6*t_hielo)
dfunc_c3 = -f2*a1*spit_x**(a2)*np.exp(-a3*f2)*np.exp(-a6*t_hielo)
dfunc_c6 = -t_hielo*a1*spit_x**(a2)*np.exp(-a3*f2)*np.exp(-a6*t_hielo)
dfunc_c4 = f1*np.exp(-a5*f2)*np.exp(-a7*t_hielo)
dfunc_c5 = -f2*a4*f1*np.exp(-a5*f2)*np.exp(-a7*t_hielo)
dfunc_c7 = -t_hielo*a4*f1*np.exp(-a5*f2)*np.exp(-a7*t_hielo)

delta_func = sqrt((dfunc_c1**(2)*sig1**(2)) + (dfunc_c3**(2)*sig3**(2)) + (dfunc_c4**(2)*sig4**(2)) + (dfunc_c5**(2)*sig5**(2)) + (dfunc_c6**(2)*sig6**(2)) + (dfunc_c7**(2)*sig7**(2)))
delta_cont = sqrt((dfunc_c1**(2)*sig1**(2)) + (dfunc_c3**(2)*sig3**(2))+ (dfunc_c6**(2)*sig6**(2)))
delta_sb   = sqrt((dfunc_c4**(2)*sig4**(2)) + (dfunc_c5**(2)*sig5**(2)) + (dfunc_c7**(2)*sig7**(2)))

#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

file400 = open('continuum_spitzer_other_method_ngc1614_plus_water2.dat','w+')
print >>file400, '#lambda(um)Rest-Frame', 'f_nu(mJy)'
for i in range(0,len(spit_x)):
  print >>file400, spit_x[i], y_cont[i]
file400.close()  
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
plt.figure()
