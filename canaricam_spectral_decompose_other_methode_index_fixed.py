...
def func(p, ccam_x):
  c1, c3, c4, c5 = p
  return c1*ccam_x**(2.0)*np.exp(-c3*f_ext) + c4*f_sb*np.exp(-c5*f_ext)
...
y_cont = p[0]*ccam_x**(a2)*np.exp(-p[1]*f_ext)
y_sb = p[2]*f_sb*np.exp(-p[3]*f_ext)
y_tot = y_cont + y_sb
...
delta_func = sqrt((dfunc_c1**(2)*sig1**(2))  + (dfunc_c3**(2)*sig3**(2)) + (dfunc_c4**(2)*sig4**(2)) + (dfunc_c5**(2)*sig5**(2)))
delta_cont = sqrt((dfunc_c1**(2)*sig1**(2))  + (dfunc_c3**(2)*sig3**(2)))
delta_sb   = sqrt((dfunc_c4**(2)*sig4**(2)) + (dfunc_c5**(2)*sig5**(2)))
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

file400 = open('continuum_CCAM_other_method_ESO244G012_c2_fixed.dat','w+')
print >>file400, '#lambda(um)Rest-Frame', 'f_nu(mJy)'
for i in range(0,len(ccam_x)):
  print >>file400, ccam_x[i], y_cont[i], delta_cont[i]
file400.close()  

file500 = open('spectrum_CCAM_other_method_ESO244G012_c2_fixed.dat','w+')
print >>file500, '#lambda(um)OBS', 'f_nu(mJy)'
for i in range(0,len(ccam_x)):
  print >>file500, ccam_x[i]*(1.+zred), ccam_y[i]-y_sb[i], sqrt(std_starb[i]**(2)+ccam_ey[i]**(2))
file500.close()  
#print 'Unresolved component'
#for i in range(0,len(ccam_x)):
#  print ccam_x[i]*(1+zred), ccam_y[i]-y_sb[i], sqrt(std_starb[i]**(2)+ccam_ey[i]**(2))
...
plt.errorbar(lccam,fccam, efccam, fmt='o',color='red',markersize=14, label='CC unresolved, present work')
plt.errorbar(l12_5, f12_5, ef12_5, fmt='o', color='orange',markersize=14, label='Soifer et al. unresolved')
...
plt.fill([9.36/(1+zred),10.04/(1+zred),10.04/(1+zred),9.36/(1+zred)],[-50.,-50.,50,50], color='Gainsboro',fill=False, hatch='\\')
...
plt.errorbar(lccam,fccam, efccam, fmt='o',color='red',markersize=10, label='CC unresolved, present work')
plt.errorbar(l12_5, f12_5, ef12_5, fmt='o', color='orange',markersize=10, label='Soifer et al. unresolved')
