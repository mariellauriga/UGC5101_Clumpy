...
#este esta mal escalado
#x_lamb, y_flux=loadtxt('../spitzer_spectrum/spitzer_spec_completo_to_ism.dat', unpack=True)

###%%%ESTE ESTA BIEN ESCALADO%555
x_lamb, y_flux, err=loadtxt('../spitzer_spectrum/spitzer_ugc5101_2.dat', unpack=True)
spit_y = y_flux*1000.
spit_x = x_lamb/(1.+zred)
...
###Este es el que da el mejor ajuste
...
delta_func = sqrt(dfunc_c1**(2)*sig1**(2) + (dfunc_c3**(2)*sig3**(2)) + (dfunc_c5**(2)*sig5**(2)))
delta_cont = sqrt( (dfunc_c1**(2)*sig1**(2)) + (dfunc_c3**(2)*sig3**(2)) )
delta_sb   = sqrt(dfunc_c5**(2)*sig5**(2) )
...
file400 = open('unresolved_spitzer_other_methode_component.dat','w+')
print >>file400, '#lambda(um)Rest-Frame', 'f_nu(mJy)-sb', 'err', 'cont'
for i in range(0,len(spit_x)):
  print >>file400, spit_x[i], spit_y[i]-y_sb[i], sqrt((0.1*spit_y[i])**(2)+ delta_sb[i]**(2)),y_cont[i]
file400.close()  
print 'Unresolved emission-Spitzer'
for i in range(0,len(spit_x)):
  print spit_x[i]*(1+zred), spit_y[i]-y_sb[i], sqrt((0.1*spit_y[i])**(2)+ delta_sb[i]**(2)), ((spit_y[i]-y_sb[i])/spit_y[i])*100
